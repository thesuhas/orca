...
fn iterator_inject_i32_before() {
    ...
    if let Err(e) = check_instrumentation_encoding(&out, file) {
        error!("Something went wrong when checking instrumentation encoding: {}", e)
    }
}
...
fn inject_semantic_after<'a, 'b, 'c>(mod_it: &mut ModuleIterator<'a, 'b>, ops_of_interest: &Vec<(SupportedOperators, Vec<Operator<'c>>)>) where 'c:'b {
    loop {
        let op = mod_it.curr_op();
        if let Location::Module {
            func_idx,
        ...
        } = mod_it.curr_loc()
        {
            ...
            for (op, body) in ops_of_interest.iter() {
                let matches = match op {
                    SupportedOperators::Br => matches!(mod_it.curr_op().unwrap(), Operator::Br {..}),
                    SupportedOperators::BrIf => matches!(mod_it.curr_op().unwrap(), Operator::BrIf {..}),
                    SupportedOperators::BrTable => matches!(mod_it.curr_op().unwrap(), Operator::BrTable {..}),
...
fn get_wat_with_inline_instrumentation(reader: &mut BufReader<File>) -> Result<String, std::io::Error> {

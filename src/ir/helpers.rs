use wasmparser::{ComponentAlias, ComponentDefinedType, ComponentExport, ComponentImport, ComponentType, ComponentTypeDeclaration, CompositeInnerType, CoreType, InstanceTypeDeclaration, ModuleTypeDeclaration, SubType};

pub fn print_alias(alias: &ComponentAlias) {
    match alias {
        ComponentAlias::InstanceExport {kind: _kind, instance_index: _instance_index, name: _name} => eprintln!("Instance Export Alias"),
        ComponentAlias::CoreInstanceExport {kind: _kind, instance_index: _instance_index, name: _name} => eprintln!("Core Instance Export Alias"),
        ComponentAlias::Outer {kind:  _kind, count: _count, index: _index} => eprintln!("Outer Alias"),
    }
}
...
pub fn print_module_ty_declaration(ty: &ModuleTypeDeclaration) {
    ...
    match ty {
        ...
        ModuleTypeDeclaration::Export {name: _name, ty: _ty} => eprintln!("Export"),
        ModuleTypeDeclaration::Import(_) => eprintln!("Import"),
        ModuleTypeDeclaration::OuterAlias {kind: _kind, count: _count, index: _index} => eprintln!("Outer Alias")
    }
}
